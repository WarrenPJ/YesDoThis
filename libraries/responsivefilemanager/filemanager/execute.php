...
//Start update database	to delete files

$info=pathinfo($path);

//Get file name with extention (is old name)
$data_path_thumbs = $path_thumb;
$file_without_path = substr($data_path_thumbs, strpos($data_path_thumbs, "thumbs/") + 7);    
$old_file_name_without_extention = substr($file_without_path, 0, strpos($file_without_path, ".")); 
//echo $old_file_name_without_extention;

//Get new name plus extention (should be the name to update the database)
$new_file_with_extension = $name . '' . ".".$info['extension'];
//echo $new_file_with_extension;

$title_undefined = "Undefined";
$default_image = "thumb-default.jpg";
...
window.location.replace("dialog.php?akey=<?php echo $private_secretkey; ?>");
</script>
<?php
...
//Check if image to update
if (".".$info['extension'] == ".jpg"){
	
$sql_video = "UPDATE video SET image = '".$default_image."' WHERE image = '".$file_without_path."'";
if ($connection->query($sql_video) === TRUE) {
	}}
...
//Check if movie file to update
if (".".$info['extension'] == ".mp4"){

$sql_video = "UPDATE video SET path = '".$title_undefined."' WHERE path = '".$old_file_name_without_extention."'";
if ($connection->query($sql_video) === TRUE) {
	}}

					
//End update database	to delete files			
...
$sql_video = "UPDATE video SET image = '".$new_file_with_extension."' WHERE image = '".$file_without_path."'";
if ($connection->query($sql_video) === TRUE) {
	}}
...
$sql_video = "UPDATE video SET path = '".$name."' WHERE path = '".$old_file_name_without_extention."'";
if ($connection->query($sql_video) === TRUE) {
	}}
