...
$url = 'http'.(isset($_SERVER['HTTPS'])?'s':'').'://'.$_SERVER['HTTP_HOST'].$dir;												?>
		...
		<form class="form-horizontal" role="form" id="paypalForm" method="post" action="<?php echo $paypal_url; ?>">
			...
			<input type="hidden" name="cancel_return" value="<?php echo $url; ?>/payment.php?type=cancel">
			<input type="hidden" name="return" value="<?php echo $url; ?>/payment.php?type=success">

			<input type="hidden" class="form-control" name="amount" placeholder="Enter Amount" required="required" value="<?php echo $price_membership; ?>">
